        ...
        protected static function validation($payloads)
        {
            ...
            foreach($payloads as $payload) {
                ...
                if ($payload->validator == 'string') {
                    if (preg_match('/[^A-Za-z0-9 ]/', $payload->data)) {
                        array_push($response, [
                            'key' => $payload->key,
                ...
                if ($payload->validator == 'handle') {
                    if (!preg_match('/^[A-Za-z0-9_]+$/', $payload->data)) {
                        array_push($response, [
                            'key' => $payload->key,
                            'message' => "Sorry {$payload->key} expect a string with no space or special characters."
                        ]);
                    }
                }
                ...
                if ($payload->validator == 'slugExists') {
                    try {
                        $UserModel = new UserModel();
                        $checkSlug = $UserModel::checkSlug($payload->data);
                        if ($checkSlug['status']) {
                            array_push($response, [
                                'key' => $payload->key,
                                'message' => "Sorry, {$payload->key} already exists. Please try with a different Slug."
                            ]);
                        }
                    } catch (Exception $e) { /** */ }
                }
